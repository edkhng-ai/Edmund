...
- Full-width gradient hero with call-to-action buttons
...
Everything - layout, styling, and behavior - lives in the one `index.html` file, organized into clearly delimited sections (navbar -> hero -> services -> testimonials -> enquiry form -> footer, followed by a script block covering footer-year injection, mobile nav toggle, scroll animations, and form handling).

## Form submissions

The enquiry form validates input and logs the submitted data to the browser console (`enquiryData` object) - it does not send data anywhere. A commented-out `fetch(...)` call marks the spot to wire up a real backend/API endpoint later.
...
Colors, fonts, and spacing are controlled by CSS custom properties defined at `:root` in `index.html` (e.g. `--color-primary`, `--color-teal`, `--color-accent`). Update those variables to re-theme the site without touching individual component styles.
